...
public class StringFinder02 {
	...
	public static void main(String[] args) throws IOException {
		...
		String grepfor = "capital";
		Path path = Paths.get("D:\\Codes and POCs\\london.txt");
	...
	private static final int MAPSIZE = 4 * 1024; // 4K - make this * 1024 to 4MB
													// in a real system.

	private static String searchFor(String grepfor, Path path)
			throws IOException {
		final byte[] tosearch = grepfor.getBytes(StandardCharsets.UTF_8);
		StringBuilder report = new StringBuilder();
		int padding = 1; // need to scan 1 character ahead in case it is a word
							// boundary.
		int linecount = 0;
		int matches = 0;
		...
		try (FileChannel channel = FileChannel.open(path,
			...
			while (pos < length) {
				...
				// int conversion is safe because of a safe MAPSIZE.. Assume a
				// reaosnably sized tosearch.
				int trymap = MAPSIZE + tosearch.length + padding;
				int tomap = (int) Math.min(trymap, remaining);
				// different limits depending on whether we are the last mapped
				// segment.
				int limit = trymap == tomap ? MAPSIZE
						: (tomap - tosearch.length);
				MappedByteBuffer buffer = channel.map(MapMode.READ_ONLY, pos,
						tomap);
