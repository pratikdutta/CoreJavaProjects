...
public class Service1 {
    ...
    public Observable<Message> getMessagesObs() {
    	return Observable.<Message>create(s -> {
    		logger.info("Start: Executing slow task in Service 1 via getMessagesObs");
    		Util.delay(500);
    		s.onNext(new Message("data 1"));
    		Util.delay(500);
    		s.onNext(new Message("data 2"));
    		Util.delay(500);
    		s.onNext(new Message("data 3"));
    		Util.delay(500);
    		s.onNext(new Message("data 4"));
    		Util.delay(500);
    		s.onNext(new Message("data 5"));
    		Util.delay(500);
    		s.onNext(new Message("data 6"));
    		Util.delay(500);
    		s.onNext(new Message("data 7"));
    		logger.info("End: Executing slow task in Service 1 via getMessagesObs");
    		s.onCompleted();
    	}).subscribeOn(Schedulers.from(customObservableExecutor));
    }
